...
def try_get_pretrained(teacher, student, raw_student, scratch):
    ...
    stu_path = '../module/poly_student_our_wo_ali.pth'
    dis_path = '../module/poly_discriminator_our_wo_ali.pth'
    if not scratch:
        ...
        for phase in iter(phases):
            ...
            for batch in tqdm(ldr):
                ...
                if phase == 'train':
                    ...
                    err_CL = max(kl_x_pos - kl_x_neg + 0.9, torch.tensor(0).cuda())

                    (errCE_wht + err_CL).backward()
                    optimizer_stu.step()
                else:
                    val_acc += "%f\n" % (acc_wht)
